...
Network must match where contract is deployed (often Ethereum Sepolia 11155111).
"""
...
# Canonical EVM transaction hash (32-byte hex, 0x-prefixed).
...
# GET only (onboarding / doctor) — avoid POST /execute/contract-call as a simple API-key ping on each install.
...
def _normalize_keeperhub_bearer_token(api_key: str) -> str:
    """
    Normalize key for ``Authorization: Bearer ...``.
    KeeperHub may provide keys as ``kh-<secret>`` or ``kh_<secret>``;
    do not prefix twice (e.g. ``kh_kh-...`` -> 401).
    """
    tok = api_key.strip()
...
def _authorization_headers(api_key: str) -> dict[str, str]:
    """KeeperHub: Bearer token (X-API-Key alone often returns 401)."""
...
async def keeperhub_anchor_registry(ph: str, rh: str, *, timeout: float = 120.0) -> dict[str, Any]:
    """
    Call KeeperHub to execute ``anchor(pattern_hash, root_hash)``.
