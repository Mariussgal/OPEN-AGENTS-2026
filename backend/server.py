...
async def get_memory_stats():
    from memory.collective_0g import _get_manifest
    try:
    ...
    # "tx" = bootstrap, "tx_hash" = contributions live
    valid_entries = [e for e in manifest if e.get("root_hash") or e.get("tx") or e.get("tx_hash")]

    recent_hits = []
    for entry in reversed(valid_entries[-50:]):
        recent_hits.append({
            ...
            "tx_hash": entry.get("tx_hash") or entry.get("tx", ""),
            "root_hash": entry.get("root_hash") or entry.get("pattern_hash", ""),
        })
